...
public:
  MPCCNode() : Node("mpcc_node") {
      ...
      // servo sub
      servo_sub_ = this->create_subscription<std_msgs::msg::Float64>("sensors/servo_position_command", 10, std::bind(&MPCCNode::servo_callback, this, _1));
      ...
      // NEW: s_hint pub & corridor sub & corridor marker pub
      s_hint_pub_ = this->create_publisher<mpcc_ros::msg::SHint>("/mpcc/s_hint", 1);
      corridor_sub_ = this->create_subscription<mpcc_ros::msg::Corridor>("/mpcc/corridor", 1, std::bind(&MPCCNode::corridor_callback, this, _1));
      corridor_marker_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("corridor_markers/mpcc", 1);

      control_pub_       = this->create_publisher<mpcc_ros::msg::MpccControl>("/mpcc/control", 10);
...
private:
  ...
  void timer_callback() {
    ...
    const double S_THR = 0.5; // 필요시 조정

    MPCReturn mpc_sol;

    if (corridor_available_ && diff < S_THR) {
      mpc_sol = mpc_->runMPCWithCorridor(latest_state_, last_corridor_);
      RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000, "Using Corridor (diff=%.3f < %.3f).", diff, S_THR);
    } 
    else {
      mpc_sol = mpc_->runMPC(latest_state_);
      if (corridor_available_) {
        RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000, "Corridor available but diff=%.3f >= %.3f, fallback to default MPC.", diff, S_THR);
      }
    }
