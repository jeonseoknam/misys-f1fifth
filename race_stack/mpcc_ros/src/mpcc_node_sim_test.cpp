...
public:
  MPCCNode() : Node("mpcc_node") {
      ...
      x0_ = {3.0, 0.5, 0.0, v0_, 0, 0, 0, 0., 0, v0_};
      // x0_ = {2.4, 0.1, 0, v0_, 0, 0, 0, 0, 0, v0_};
      ...
      corridor_sub_ = this->create_subscription<mpcc_ros::msg::Corridor>("/mpcc/corridor", 1, std::bind(&MPCCNode::corridor_callback, this, std::placeholders::_1));
...
private:
  ...
  void timer_callback() {
    ...
    std::cout << "s: " << x0_.s << std::endl;

    // // 1) 초기추정만 준비하고 SHint 발행
    // last_s_hint_ = mpc_->prepareInitialGuess(x0_);
    // mpcc_ros::msg::SHint hint;
    // hint.s = last_s_hint_;
    // hint.x = x0_.X; hint.y = x0_.Y; 
