...
from on_track_sys_id.generate_predictions import generate_predictions
from on_track_sys_id.generate_inputs_errors import generate_inputs_errors
from on_track_sys_id.SimpleNN import SimpleNN
from on_track_sys_id.pacejka_formula import pacejka_formula
from on_track_sys_id.plot_results import plot_results
from on_track_sys_id.solve_pacejka import solve_pacejka
from on_track_sys_id.save_model import save
from on_track_sys_id.load_model import get_dotdict
from on_track_sys_id.simulate_model import LookupGenerator
...
# virtual steady-state data generation 하는 부분
...
def get_model_param(racecar_version):
    ...
    yaml_file = os.path.join(package_path, 'models', racecar_version + '.txt')
    with open(yaml_file, 'r') as file:
...
def nn_train(training_data, racecar_version, save_LUT_name, plot_model):
    ...
    logger = get_logger('NN_Trainer')  # 로거 객체 생성
    ...
    for i in range(1, num_of_iterations+1):
        ...
        for epoch in range(1, num_of_epochs+1):
            ...
            if (epoch == num_of_epochs):
                ...
                # least squre 기법을 통해 Pacejka 계수를 추정하는 부분
                ...
                if plot_model:
                    logger.warn("Close the plot window (press Q) to continue... ")
                    plot_results(model, v_x, v_y, omega, delta, C_Pf_identified, C_Pr_identified, i)   
                    
                # norminal vehicle model의 Pacejka 계수를 업데이트하는 부분
