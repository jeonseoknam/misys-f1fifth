...
#include "f110_msgs/msg/purepursuit_control.hpp"      // steering_angle, speed

class TopController : public rclcpp::Node {
public:
  TopController()
  : rclcpp::Node("top_controller")
  {
    // ---- 파라미터 ----
    loop_rate_hz_ = this->declare_parameter<double>("loop_rate_hz", 40.0);

    // 0-based 인덱스: 기본 3 = '4번 버튼'
    ...
    // ---- 퍼블리셔 ----
    duty_pub_  = this->create_publisher<std_msgs::msg::Float64>("/commands/motor/duty_cycle", 10);
    ...
    mpcc_sub_ = this->create_subscription<mpcc_ros::msg::MpccControl>(
      "/mpcc/control", rclcpp::QoS(10),
      std::bind(&TopController::on_mpcc, this, std::placeholders::_1));

    pp_sub_ = this->create_subscription<f110_msgs::msg::PurepursuitControl>(
      "/purepursuit/control", rclcpp::QoS(10),
      std::bind(&TopController::on_pp, this, std::placeholders::_1));
...
private:
  ...
  void on_mpcc(const mpcc_ros::msg::MpccControl::SharedPtr msg) {
    ...
    // 최근 10개 solver_status==0 여부를 기록
    const bool ok = (msg->solver_status == 0);
    mpcc_ok_hist_.push_back(ok);
    if (mpcc_ok_hist_.size() > hist_len_) mpcc_ok_hist_.pop_front();
  }

  void on_pp(const f110_msgs::msg::PurepursuitControl::SharedPtr msg) { last_pp_ = *msg; }

  void on_joy(const sensor_msgs::msg::Joy::SharedPtr msg) {
    const auto &btns = msg->buttons;

    // 첫 프레임 토글 방지: 처음 수신이면 상태만 저장하고 리턴
    if (!last_buttons_) {
      last_buttons_ = btns;
      return;
    }

    // JOY 토글 (한 버튼으로 AUTO<->JOY 토글)
  ...
  void loop() {
    switch (mode_) {
      case ControlMode::JOY_ARMING:
        ...
        if (this->get_clock()->now() < joy_stop_until_) {
          ...
          RCLCPP_INFO(get_logger(), "JOY arming done -> Mode=JOY (top_controller stops publishing /drive)");
          return; // 이번 틱은 퍼블리시 없이 종료 (다음 틱부터 완전 JOY)
        }

      case ControlMode::JOY:
        // JOY 모드: top_controller는 /drive 퍼블리시 안 함 (joy_teleop가 퍼블리시)
        return;
    ...
    // TRAILING 상태일 때는 PP 제어 우선
    if (state_ == "StateType.TRAILING") {
      if (last_pp_) {
        publish_ackermann_from_pp(*last_pp_);
      } else {
        RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                              "State=TRAILING but no /purepursuit/control yet.");
      }
      return;
    }

    // MPCC 안정성 검사 후 사용 (필요 시 활성화)
    // if (mpcc_is_stable()) {
    //   publish_low_level_from_mpcc(*last_mpcc_);
    //   return;
    // }

    // MPCC 비안정(또는 미수신) → PP 사용
    if (last_pp_) {
      publish_ackermann_from_pp(*last_pp_);
    } else {
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                           "AUTO: No valid control: MPCC not stable and no /purepursuit/control yet.");
    }
  }
  ...
  bool mpcc_is_stable() const {
    ...
    if (mpcc_ok_hist_.size() < hist_len_) return false;
  ...
  void publish_ackermann_from_pp(const f110_msgs::msg::PurepursuitControl &pp) {
    ackermann_msgs::msg::AckermannDriveStamped ack;
    ack.header.stamp = now();
    ack.header.frame_id = "base_link";
    ack.drive.steering_angle = pp.steering_angle;
    ack.drive.speed = pp.speed;
...
private:
  ...
  int joy_toggle_button_idx_{5};   // 0-based index
  ...
  rclcpp::Subscription<f110_msgs::msg::PurepursuitControl>::SharedPtr pp_sub_;
  ...
  std::optional<f110_msgs::msg::PurepursuitControl> last_pp_;
  ...
  // MPCC 안정성 히스토리 (최근 5개)
  static constexpr size_t hist_len_ = 5;
